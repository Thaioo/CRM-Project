...
public Timestamp formatTimeStamp(String dateStr) {
    // Define the date formats to be tried
    String[] dateFormats = {
        "dd/MM/yyyy",
        "yyyy-MM-dd HH:mm:ss",
        "yyyy-MM-dd",
    ...
    for (String dateFormat : dateFormats) {
        try {
            // Create a SimpleDateFormat object for each date format
            ...
            // Continue to the next date format if parsing fails
            continue;
        }
    }

    if (timestamp == null) {
        // Handle the case when none of the date formats work
...
private void moveTaskAdd(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
	List<StatusEntity> li = taskService.getAllListStatus();
